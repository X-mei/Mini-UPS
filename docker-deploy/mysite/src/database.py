...
#     def __init__(self):
#         try:
#             self.conn = psycopg2.connect(database='postgres', user='postgres', password='passw0rd', host='127.0.0.1', port='5432')
...
#create a package
def create_package(package_id, wh_id, truck_id, user_id, x, y, products):
    conn = db.connect()
    cur = conn.cursor()
    status = 'packing'
    if user_id == -1:
    ...
    conn.commit()
    for prod in products:
        create_product(conn, prod.id, prod.description, prod.count, package_id)
    conn.close()


#create a product
def create_product(conn, prod_id, prod_desc, prod_cnt, package_id):
    cur = conn.cursor()
    cur.execute("INSERT INTO ups_product (product_id, product_description, product_count, product_package_id) \
                    VALUES (%s, %s, %s, %s);", (prod_id, prod_desc, prod_cnt, package_id, ))
    conn.commit()
